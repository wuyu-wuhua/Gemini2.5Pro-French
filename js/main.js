...
function getSafeTranslations() {
    ...
    return {
        ...
        fr: { 'untitled-chat': 'Без названия чата', 'history-title': 'История чата', 'no-history': 'Еще нет истории', 'welcome-message': 'Привет! Чем могу помочь?' }
    };
}
...
function switchLanguage(lang) {
    if (lang && lang === 'fr') { // Only fr is supported now
...
function applyTranslations(languageToApply) {
    const currentLang = languageToApply || localStorage.getItem('language') || 'fr';
    const trans = getSafeTranslations();
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    langOptions.forEach(option => {
        ...
        // Since only fr is supported, ensure only fr option can be active if it exists
        if (option.getAttribute('data-lang') !== 'fr') option.style.display = 'none'; else option.style.display = 'block';
    });
...
if (langOptions) {
    langOptions.forEach(option => {
        ...
        newOption.addEventListener('click', (e) => {
            ...
            if (lang === 'fr') { // Only allow switching to fr
                switchLanguage(lang);
            }
        });
    });
